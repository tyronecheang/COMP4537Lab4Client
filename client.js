...
class LabAPI {
  constructor() {
    this.API_BASE_URL = "https://comp4537lab4.duckdns.org/lab4/api/v1";
...
class LabUI {
  ...
  buildUI() {
    ...
    insertBtn.addEventListener("click", async () => {
      ...
      try {
        insertResponse.textContent = await this.api.insertPatients();
      } catch (err) {
        insertResponse.textContent = STRINGS.errorPrefix + err.message;
      }
    });
    ...
    queryBtn.addEventListener("click", async () => {
      ...
      try {
        queryResponse.textContent = await this.api.runQuery(query);
      } catch (err) {
        queryResponse.textContent = STRINGS.errorPrefix + err.message;
      }
    });
  }
